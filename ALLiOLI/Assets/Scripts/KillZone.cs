...
{
    [Tooltip("Not mandatory")] [SerializeField]
    private float minimumVelocityToKill = 0.75f;
    ...
    private void OnCollisionEnter(Collision other)
    {
        ...
        if (rigidbody)
            if (rigidbody.velocity.magnitude < minimumVelocityToKill)
            {
                ...
                Debug.Log(
                    chr.name + " avoided being killed by" + transform.gameObject.name + "\nVelocity = " +
                    rigidbody.velocity.magnitude + " (required " + minimumVelocityToKill + " to kill)", gameObject);
                return;
            }
