...
public class ThirdPersonCamera : MonoBehaviour
{
    ...
    [Space] [SerializeField] private LayerMask layersThatCanClipCamera;

    [SerializeField] private float maxPitch = 75;

    [Space] [SerializeField] private float minPitch = 5;

    [HideInInspector] public Vector2 movement;
    [SerializeField] private float offsetOnCollision = 0.5f;
    [SerializeField] private float pitchRotationalSpeed = 200;

    [Space] [Tooltip("Not necessary to set if the setup is set trough code")] [SerializeField]
    private Transform target;

    [Space] [SerializeField] private float yawRotationalSpeed = 200;
    ...
    private void LateUpdate()
    {
        ...
        pitch *= Mathf.Deg2Rad;
        Vector3 desiredPosition = target.position + new Vector3(Mathf.Sin(yaw) * Mathf.Cos(pitch) * distanceToTarget,
                                      Mathf.Sin(pitch) * distanceToTarget,
                                      Mathf.Cos(yaw) * Mathf.Cos(pitch) * distanceToTarget);
